...
class MoRController(nn.Module):
    def __init__(self, d_model, max_depth=3):
        super().__init__()
        self.max_depth = max_depth
        self.fc1 = nn.Linear(d_model, d_model)
        self.fc2 = nn.Linear(d_model, max_depth)

    def forward(self, hidden_states, attention_mask=None):
        scores = self.fc2(F.relu(self.fc1(hidden_states)))
        if self.training:
            probs = F.gumbel_softmax(scores, tau=1.0, hard=False, dim=-1)
        else:
            # 推理时强制选择最可能的深度（hard决策）
            probs = F.one_hot(torch.argmax(scores, dim=-1), num_classes=self.max_depth).float()
        return probs
...
class GatingNetwork(nn.Module):
    def __init__(self, config, num_experts, top_k=3):
        super().__init__()
    ...
    def forward(self, hidden_states, attention_mask=None):
        ...
        self.last_gate_full = full_probs.detach().clone()
        ...
        self.last_gate_weights = probs.detach().clone()
        return probs


class MoEBlock(nn.Module):
    def __init__(self, config, experts, gating, top_k=3, max_depth=3):
        super().__init__()
        ...
        self.gating = gating
        self.top_k = top_k
        self.mor_controller = MoRController(config.d_model, max_depth)
        self.max_depth = max_depth

    def forward(self, hidden_states, attention_mask=None):
        normed = self.layer_norm(hidden_states)  # [B, L, D]
        residual = normed

        depth_probs = self.mor_controller(normed, attention_mask)  # 连续可微权重

        output = normed

        for step in range(self.max_depth):
            mask = depth_probs[:, :, step].unsqueeze(-1)  # [B, L, 1]，权重

            gate_logits = self.gating(output, attention_mask)  # [B, L, E]
            topk_vals, topk_indices = torch.topk(gate_logits, self.top_k, dim=-1)
            topk_weights = torch.softmax(topk_vals, dim=-1)

            moe_output = torch.zeros_like(residual)
            ...
            for k in range(self.top_k):
                ...
                for i, expert in enumerate(self.experts):
                    ...
                    if mask_i.sum() == 0:
                        continue
                    selected = output[mask_i]  # [N, D]

                    selected_mask = attention_mask[mask_i] if attention_mask is not None else None
                    expert_out = expert(
                        selected.unsqueeze(0),
                        selected_mask.unsqueeze(0) if selected_mask is not None else None
                    )[0].squeeze(0)

                    moe_output[mask_i] += expert_out * expert_weight[mask_i].unsqueeze(-1)

            residual = mask * moe_output + (1 - mask) * residual

        return hidden_states + self.dropout(residual)


class MoEEncoder(PreTrainedModel):
    main_input_name = "input_ids"

    def __init__(self, config, shared, experts, gating, num_layers=6, top_k=2, max_depth=3):
        super().__init__(config)
        self.shared = shared
        self.layers = nn.ModuleList([
            MoEBlock(config, experts, gating, top_k=top_k, max_depth=max_depth)
    ...
    def forward(
        ...
        gates_per_layer = [] if return_gates else None
        ...
        for layer in self.layers:
            ...
            if return_gates:
                layer_gating = getattr(layer, 'gating', None)
                if layer_gating is None:

                    any_gating = None
                    for ly in self.layers:
                        any_gating = getattr(ly, 'gating', None)
                        if any_gating is not None:
                            break
                    if any_gating is None:
                        raise RuntimeError("No gating found in any MoEBlock; cannot collect gates.")
                    num_experts = any_gating.classifier.out_features
                    bsz = hidden_states.size(0)
                    seq_len = hidden_states.size(1)
                    gates_per_layer.append(torch.zeros(bsz, seq_len, num_experts))
                else:

                    if hasattr(layer_gating, 'last_gate_full') and layer_gating.last_gate_full is not None:
                        gates_per_layer.append(layer_gating.last_gate_full.detach().clone())
                    else:
                        last_gate = getattr(layer_gating, 'last_gate_weights', None)
                        if last_gate is None:
                            num_experts = layer_gating.classifier.out_features
                            bsz = hidden_states.size(0)
                            seq_len = hidden_states.size(1)
                            gates_per_layer.append(torch.zeros(bsz, seq_len, num_experts))
                        else:
                            gates_per_layer.append(last_gate.detach().clone())
        ...
        if return_gates:
            return base_out, gates_per_layer
...
class HFMoET5Model(T5PreTrainedModel, GenerationMixin):
    def __init__(self, config, num_experts=6, num_layers=6, top_k=2, max_depth=3):
        super().__init__(config)
        ...
        self.encoder = MoEEncoder(config, self.shared, self.experts, self.gating,
                                  num_layers=num_layers, top_k=top_k, max_depth=max_depth)
    ...
    def get_gates_for_inputs(self, input_ids, attention_mask=None, device=None):
        """
        Run encoder with return_gates=True and return gates_per_layer (list of tensors).
        Each tensor in the list has shape [batch, seq_len, num_experts].
        ...
        with torch.no_grad():
            ...
            # call encoder with return_gates=True
            encoder_out, gates_per_layer = self.encoder(
                input_ids,
                attention_mask=attention_mask,
                output_hidden_states=False,
                return_dict=True,
            ...
            gates_cpu = [g.detach().cpu() for g in gates_per_layer]
        return gates_cpu
...
def main(args):
    ...
    for epoch in range(args.epoch):
        ...
        for idx, (inputs, atts, labels) in enumerate(tqdm(train_dataloader)):
            ...
            outputs = model(input_ids=inputs, attention_mask=atts, decoder_input_ids=None, labels=labels,
                            return_dict=True)
